...
function BuatGambarKonveksiUndercut(req, res) {
  const produk = req;
  let csize = {
    w: 720, //Lebar
    l: 200, //Panjang
  ...
  produk.forEach((element) => {
    if (element.pesaing[0].hargapesaing != "-") {
      if (parseInt(element.pesaing[0].hargapesaing) < parseInt(element.hargaproduk)) {
        tambahukuran = 1;
      }
    }
    if (element.pesaing[1].hargapesaing != "-") {
      if (parseInt(element.pesaing[1].hargapesaing) < parseInt(element.hargaproduk)) {
        tambahukuran = 1;
      }
    }
    if (tambahukuran == 1) {
