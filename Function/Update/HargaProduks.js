...
async function ScrapDataProduk(req, res) {
  ...
  data.hargaproduk = value0.replace(/Rp/g, "").replace(/ /g, "").replace(".", "").replace(".", ""); // update data harga produk dengan value yang didapat
  ...
  if (data.pesaing[0].linkpesaing != "-") {
    ...
    data.pesaing[0].hargapesaing = value1.replace(/Rp/g, "").replace(/ /g, "").replace(".", "").replace(".", "");
  ...
  if (data.pesaing[1].linkpesaing != "-") {
    ...
    data.pesaing[1].hargapesaing = value2.replace(/Rp/g, "").replace(/ /g, "").replace(".", "").replace(".", "");
