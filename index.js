...
const ApiName = require("./Function/Generator/Products/Updates/ApiName");

// Notifikasi
const { AutoSelesai, AutoMulai, AutoUndercut } = require("./Function/Update/APIProducts.js");
const { KonveksiSelesai } = require("./Function/Update/APIKonveksi");
...
async function Kodommo() {
    ...
    if (msg.body.startsWith("!")) {
      switch (true) {
        ...
        case msg.body.toLowerCase().startsWith("!link"): // Cek Produk
          ...
          break;
        case msg.body.toLowerCase().startsWith("!api"): // Cek Produk
          Generate = await ApiName(msg.body, (await msg.getContact()).number);
          msg.reply(Generate);
          break;
  ...
  // setInterval(UpdatePesaing, 10000);
  async function UpdatePesaing() {
    let Undercuted = await AutoUndercut();
    for (let i = 0; i < Undercuted.length; i++) {
      WaBot.sendMessage(Undercut[i].nomor, Undercut[i].caption);
    }
  }
